namespace Supercluster_Workbench
{
    ...
    public class ZigguratGaussianSampler
    {
        ...
        /// </summary>
        public ZigguratGaussianSampler(Random rng)
        {
            this._rng = rng;
            ...
            this._x = new double[__blockCount + 1];
            this._y = new double[__blockCount];
            ...
            this._x[0] = __R;
            this._y[0] = this.GaussianPdfDenorm(__R);
            ...
            this._x[1] = __R;
            this._y[1] = this._y[0] + (__A / this._x[1]);

            // Calc positions of all remaining rectangles.
            for (int i = 2; i < __blockCount; i++)
            {
                this._x[i] = this.GaussianPdfDenormInv(this._y[i - 1]);
                this._y[i] = this._y[i - 1] + (__A / this._x[i]);
            }

            // For completeness we define the right-hand edge of a notional box 6 as being zero (a box with no area).
            this._x[__blockCount] = 0.0;

            // Useful precomputed values.
            this._A_Div_Y0 = __A / this._y[0];
            this._xComp = new uint[__blockCount];
            ...
            this._xComp[0] = (uint)(((__R * this._y[0]) / __A) * (double)uint.MaxValue);

            for (int i = 1; i < __blockCount - 1; i++)
            {
                this._xComp[i] = (uint)((this._x[i + 1] / this._x[i]) * (double)uint.MaxValue);
            }
            this._xComp[__blockCount - 1] = 0;  // Shown for completeness.
            ...
            // point arithmetic.
            Debug.Assert(Math.Abs(1.0 - this._y[__blockCount - 1]) < 1e-10);
        }
        ...
        public double NextSample()
        {
            for (;;)
            {
                ...
                this._rng.NextBytes(byteArray);
                byte u = byteArray[0];
                int i = (int)(u & 0x7F);
                double sign = ((u & 0x80) == 0) ? -1.0 : 1.0;

                // Generate uniform random value with range [0,0xffffffff].
                int u1 = this._rng.Next(int.MinValue, int.MaxValue);
                uint u2;
                if (u1 < 0)
                {
                    u2 = int.MaxValue + (uint)Math.Abs(u1);
                }
                else
                {
                    u2 = (uint)u1;
                ...
                if (0 == i)
                {
                    ...
                    {   // Generated x is within R0.
                        return u2 * __UIntToU * this._A_Div_Y0 * sign;
                    }
                ...
                {   // Generated x is within the rectangle.
                    return u2 * __UIntToU * this._x[i] * sign;
                }
                ...
                double x = u2 * __UIntToU * this._x[i];
                if (this._y[i - 1] + ((this._y[i] - this._y[i - 1]) * this._rng.NextDouble()) < this.GaussianPdfDenorm(x))
                {
                    return x * sign;
                }
            }
        ...
        {
            return mu + (this.NextSample() * sigma);
        }

        public double[] NextSamples(double mu, double sigma, int samples)
        {
            var samplesList = new List<double>();
            for (int i = 0; i < samples; i++)
            {
                samplesList.Add(this.NextSample(mu, sigma));   
            }
        ...
        private double SampleTail()
        {
            ...
            do
            {
                ...
                x = -Math.Log(this._rng.NextDouble() + double.Epsilon) / __R;
                y = -Math.Log(this._rng.NextDouble() + double.Epsilon);
            }
            while (y + y < x * x);
            return __R + x;
        }

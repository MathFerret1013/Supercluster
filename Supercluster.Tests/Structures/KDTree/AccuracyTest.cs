...
namespace KDTreeTests
{
    ...
    using Supercluster.Tests.DataStructures.KDTree;
    ...
    public class AccuracyTest
    {
        ...
        public void FindNearestNeighborTest()
        {
            ...
            var tree = new KDTree<double, string>(2, treePoints, treeNodes, Utilities.Taxicab_Double);

            for (int i = 0; i < testDataSize; i++)
            {
                var treeNearest = tree.NearestNeighbors(testData[i], 1);
                var linearNearest = Utilities.LinearSearch(treePoints, treeNodes, testData[i], Utilities.Taxicab_Double);

                Assert.That(Utilities.Taxicab_Double(testData[i], linearNearest.Item1), Is.EqualTo(Utilities.Taxicab_Double(testData[i], treeNearest[0].Item1)));
        ...
        public void RadialSearchTest()
        {
            ...
            var treeNodes = Utilities.GenerateDoubles(dataSize, range).Select(d => d.ToString()).ToArray();
            var testData = Utilities.GenerateDoubles(testDataSize, range);
            var tree = new KDTree<double, string>(2, treeData, treeNodes, Utilities.Taxicab_Double);
